...
export function PreviewSurface({ previewUrl, onPreviewUrlChange }: PreviewSurfaceProps) {
  ...
  const frameStyle = useMemo(() => {
    if (deviceMode === "iPhone 14 Pro Max" || deviceMode === "Safari" || deviceMode === "Chrome" || deviceMode === "Safe Zone") {
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column", background: "#07080b" }}>
      <div style={{
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") onPreviewUrlChange(localUrl || "/preview")
          }}
          style={{
            marginLeft: "auto",
            minWidth: 180,
        ...
        />
        <button
          onClick={() => onPreviewUrlChange(localUrl || "/preview")}
          style={{
            border: "1px solid rgba(255,255,255,0.08)",
            background: "rgba(255,255,255,0.04)",
            color: "rgba(255,255,255,0.82)",
        ...
        {innerMode === "preview" ? (
          <div style={{ flex: 1, display: "grid", placeItems: "center", padding: 16, overflow: "auto" }}>
            <div style={{
              ...
              <iframe src={previewUrl || "/preview"} title="Preview" style={{ width: "100%", height: "100%", border: "none", display: "block" }} />
            </div>
          </div>
        ) : innerMode === "diff" ? (
          <SplitSurface left={<iframe src={previewUrl || "/preview"} title="Preview" style={{ width: "100%", height: "100%", border: "none" }} />} right={<PlaceholderCard title="Staged Diff Review" body="Diff review lives inside the current preview panel rather than opening a new outer panel." />} />
        ) : (
          <SplitSurface left={<iframe src={previewUrl || "/preview/html"} title="Document" style={{ width: "100%", height: "100%", border: "none" }} />} right={<PlaceholderCard title="Document / Artifact Viewer" body="Documents, generated HTML, and exported artifacts are surfaced here inside the preview panel." />} />
        )}
      </div>
    </div>
  )
